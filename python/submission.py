...
def to_HTCondor(*, analyze, opt, submit_mode, plot_version, samples_to_process):
    if opt.BATCH and not opt.RUN:
        ...
        for sample in samples_to_process:
            ...
            rootfilereg = re.compile(r".*\.root$")
            inputfileslist = os.listdir(os.path.join(sample.input_base_dir, sample.input_sample_dir))
            # Count the number of root files in the input directory
            n_jobs = len([f for f in inputfileslist if rootfilereg.match(f)]) 
            ...
            #     debug=int(opt.DEBUG),
            # )
            print(f"Max # of events per file: {sample.events_per_job}")
            if n_jobs == 0:
                n_jobs = 1
            ...
            params["TEMPL_NEVENT"] = str(sample.events_per_job)
